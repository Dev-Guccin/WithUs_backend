...
router.get('/detail/:TB_code', function(req, res, next) {
    ...
    var sql = 'SELECT TeamBoard.User_code, TB_code, CT_code, TB_title, TB_recruitNumber, TB_finalNumber, TB_content, TB_createDate, TB_finalDate, TB_contestOrProject, TB_CBcode, User_nickname, CB_title FROM withus.TeamBoard left join withus.User ON TeamBoard.User_code = User.User_code left join withus.CompeteBoard on TeamBoard.TB_CBcode = CompeteBoard.CB_code  WHERE TB_code='+TB_code+';';
